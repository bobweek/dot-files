...
( function () {
	...
	specialCharList.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			//once special chars available, fetch generalOptions and redirection schemes from local storage and update them here. Must load format strings prior to schemes!
			...
			for (let i = 0; i<requests.length; ++i) {
				...
				chrome.storage.local.get(reqString, function(options) {
	...
	chrome.storage.local.get("displayOptions", function(options) {
			if (chrome.runtime.lastError) {
				console.error(chrome.runtime.lastError);
			} else {
